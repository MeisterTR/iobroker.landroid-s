...
LandroidCloud.prototype.retrieveAwsCert = function (updateListener) {
    ...
    this.worx('GET', "users/certificate", null, function (data) {
        ...
        self.worx('GET', "product-items", null, function (data) {
            ...
            if (data[1]) {
                self.adapter.log.error("More than one Mower registraded please send the whole next message to the developer");
                self.adapter.log.error(JSON.stringify(data));
            }
            self.macAddress = data[0].mac_address;
            self.adapter.log.info("Mac adress set to: " + data[0].mac_address);
