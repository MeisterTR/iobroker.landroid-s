...
var defaultPath = 'C:\\OpenSSL-Win32\\bin\\...';
...
function LandroidCloud(adapter) {
    ...
    this.uuid = uuidv1();
    this.device;
    this.cPath = adapter.config.sslpathdef;
    if (process.platform === 'win32') this.sslBool = this.openSSLonWin();

    //this.adapter.log.info("UUID: " + this.uuid);
}

LandroidCloud.prototype.openSSLonWin = function () {
    
    if (process.platform === 'win32' && fs.existsSync(this.cPath)) {
        
        process.env.OPENSSL_CONF = path.join(path.dirname(this.cPath) + '/openssl.cfg');
        this.adapter.log.info("OpenSSL path " + process.env.OPENSSL_CONF);       
        pem.config({
            pathOpenSSL: path.join(this.cPath)
        });
        return true;
    }
    else {
